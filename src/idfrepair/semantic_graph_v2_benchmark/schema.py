"""定义 builder/runtime/scorer 三进程边界使用的独立 records。"""
...
@dataclass(frozen=True, slots=True)
class MutationSupportObject:
    """Builder-only object injected to establish a controlled oracle witness."""
    ...
    with path.open("w", encoding="utf-8") as handle:
        for row in rows:
            ...
            payload["supporting_objects"] = [
                asdict(obj) for obj in row.supporting_objects
            ]
