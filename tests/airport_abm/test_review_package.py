...
def write_v31_allowlist(root: Path, paths: list[str]) -> None:
    destination = root / "docs/research/occupancy_v31/review_package_allowlist.json"
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(
        json.dumps(
            {
                "schema_version": "idfrepair.airport-abm-review-allowlist.v31",
                "paths": sorted(paths),
            }
        ),
        encoding="utf-8",
...
def test_v31_review_zip_contains_only_registered_public_evidence(
    tmp_path: Path,
) -> None:
    from tools.airport_abm.review_package_v31 import build_review_zip

    root = tmp_path / "repo"
    files = {
        "docs/research/occupancy_v31/method_boundary.md": "controlled method\n",
        "reports/occupancy_v31/final_status.md": "AIRPORT_V31_DEMO_ONLY\n",
        "src/idfrepair/analysis/airport_abm/v31.py": "VALUE = 31\n",
        "tests/airport_abm/test_v31.py": "def test_ok(): pass\n",
        "scripts/airport_abm/run_v31.py": "print('review')\n",
        "private/model.osm": "private\n",
        "private/weather.epw": "private\n",
    }
    reviewed = [name for name in files if not name.startswith("private/")]
    for name, value in files.items():
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(value, encoding="utf-8")
    write_v31_allowlist(root, reviewed)
    destination = tmp_path / "review-v31.zip"
    ...
    assert first == second
    assert first["bytes"] < 15 * 1024 * 1024
    with ZipFile(destination) as archive:
        names = archive.namelist()
        manifest_name = next(name for name in names if name.endswith("PACKAGE_MANIFEST.json"))
        manifest = json.loads(archive.read(manifest_name))
    assert manifest["schema_version"] == "idfrepair.airport-abm-review-package.v31"
    assert manifest["raw_private_models_included"] is False
    assert not any(name.endswith((".osm", ".idf", ".epw", ".sql")) for name in names)


def test_v31_review_package_rejects_an_absolute_user_path(tmp_path: Path) -> None:
    from tools.airport_abm.review_package_v31 import build_review_zip

    root = tmp_path / "repo"
    relative = "reports/occupancy_v31/final_status.md"
    path = root / relative
    path.parent.mkdir(parents=True)
    path.write_text("/" + "Users/alice/private/model.osm", encoding="utf-8")
    write_v31_allowlist(root, [relative])

    with pytest.raises(ValueError, match="review_package_sensitive_content"):
        build_review_zip(root, tmp_path / "review-v31.zip")
